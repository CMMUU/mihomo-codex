...
def cargo_components(lock: dict, metadata: dict) -> list[dict]:
    ...
    for locked in lock["package"]:
        ...
        components.append({
            ...
            "properties": [
                {"name": "mihomo-codex:ecosystem", "value": "cargo"},
                {"name": "mihomo-codex:declared-license", "value": package["license"]},
                {"name": "mihomo-codex:inventory-scope", "value": "all locked platforms and dependency kinds"},
            ],
        })
    return components
...
def npm_components(lock: dict) -> list[dict]:
    ...
    for location, package in sorted(lock["packages"].items()):
        ...
        components.append({
            ...
            "properties": [
                {"name": "mihomo-codex:ecosystem", "value": "npm"},
                {"name": "mihomo-codex:declared-license", "value": package["license"]},
                {"name": "mihomo-codex:lockfile-location", "value": location},
                {"name": "mihomo-codex:development", "value": str(package.get("dev", False)).lower()},
                {"name": "mihomo-codex:optional", "value": str(package.get("optional", False)).lower()},
                {"name": "mihomo-codex:os", "value": ",".join(package.get("os", ["any"]))},
                {"name": "mihomo-codex:cpu", "value": ",".join(package.get("cpu", ["any"]))},
            ],
        })
    return components
...
def core_component(manifest: dict) -> dict:
    ...
    return {
        ...
        "externalReferences": refs,
        "properties": [
            {"name": "mihomo-codex:ecosystem", "value": "bundled-core"},
            {"name": "mihomo-codex:declared-license", "value": manifest["license"]},
            {"name": "mihomo-codex:inventory-scope", "value": "pinned upstream core; internal Go dependency graph is outside this lockfile inventory"},
        ],
    }
...
def markdown(bom: dict, inputs: dict[str, str]) -> str:
    ...
    groups = Counter(property_value(item, "mihomo-codex:ecosystem") for item in components)
    lines = [
        f"# mihomo-codex v{version} license inventory", "",
    ...
    for ecosystem, heading in [("bundled-core", "Bundled Mihomo core"), ("cargo", "Rust / Cargo"), ("npm", "JavaScript / npm")]:
        ...
        for item in components:
            if property_value(item, "mihomo-codex:ecosystem") != ecosystem:
                continue
            license_text = property_value(item, "mihomo-codex:declared-license").replace("|", "\\|")
...
def main() -> None:
    ...
    components.sort(key=lambda item: (property_value(item, "mihomo-codex:ecosystem"), item["name"], item["version"], item["bom-ref"]))
    ...
    bom = {
        ...
        "metadata": {
            "component": {
                "type": "application", "bom-ref": "application:mihomo-codex",
                "name": package["name"], "version": package["version"],
                "licenses": [{"expression": package["license"]}],
                "externalReferences": [
                    {"type": "vcs", "url": "https://github.com/CMMUU/mihomo-codex"},
                    {"type": "vcs", "url": "https://gitee.com/cmmuu/mihomo-codex"},
                ],
            },
            "properties": [
                {"name": "mihomo-codex:inventory-scope", "value": "locked Rust/npm packages across platforms including optional/build/dev packages, plus pinned Mihomo core; excludes system libraries, platform WebViews and upstream core's internal Go graph"},
                *[{"name": f"mihomo-codex:input-sha256:{path}", "value": checksum} for path, checksum in inputs.items()],
            ],
        },
